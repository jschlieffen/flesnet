...
void Application::run() {
  ...
  if (par_.tsa_archives.size() == 2) {
    ...
    if (!par_.skip_metadata) {
      valid = val.verify_ts_metadata(par_.output_archives, &timeslice_cnt,
                                     par_.timeslice_size, par_.overlap,
                                     components_cnt);
      if (!valid) {
        throw runtime_error(
            "Output metadata check FAILED. Archives NOT VALID!");
      }
    }

    valid = val.verify_forward(par_.input_archives, par_.output_archives,
                               timeslice_cnt, par_.overlap);
    if (!valid) {
