...
// Macro to print floats with a fixed number of digits after the decimal point
// and resets the output stream to default behavior again
#define STREAM_FLOAT(p, f)                                                     \
  setprecision(p) << fixed << (f) << setprecision(-1) << defaultfloat
...
int compare_ts(fles::Timeslice& ts_a, fles::Timeslice& ts_b) {
  if (ts_a.index() != ts_b.index()) {
    L_(error) << "Timeslices have different index: " << '\n'
              << "A TS" << ts_a.index() << "B TS: " << ts_b.index();
    return -1;
  }
  ...
  for (uint64_t c = 0; c < ts_a.num_components(); c++) {
    if (ts_b.num_microslices(c) != ts_a.num_microslices(c)) {
      L_(error) << "Timeslice components " << c
                << " have different amount of microslices: " << "A TS"
                << ts_a.num_microslices(c)
...
uint64_t Verificator::find_tsa_b_in_a(const std::string& archive_a,
  ...
  unique_ptr<fles::TimesliceSource> source_a =
      make_unique<fles::TimesliceInputArchive>(archive_a);
  unique_ptr<fles::TimesliceSource> source_b =
      make_unique<fles::TimesliceInputArchive>(archive_b);
...
bool Verificator::verify_ts_forwarding(
    const std::vector<std::string>& input_archives,
    const std::vector<std::string>& output_archives) {
  ...
  for (auto const& input_path : input_archives) {
    ...
    auto in_archive =
        archive_bundle({input_path, std::move(input_archive), nullptr, 0, 0});

    std::vector<archive_bundle> output_sinks;
    for (auto const& output_path : output_archives) {
      auto output_archive =
          make_unique<fles::TimesliceInputArchive>(output_path);
      // auto ts = output_archive->get();
      output_sinks.push_back(
          {output_path, std::move(output_archive), nullptr, 0, 0});
    }

    // it is allowed that at the start of the archive, some TS may be skipped so
    // we consume in_archive.source until in_archive.curr_ts contains the first
    // transmitted timeslice
    L_(info) << "Searching for first transmitted timeslice of: "
             << in_archive.source_path << " ...";
    bool found_beginning = false;
    while (!found_beginning &&
           (in_archive.curr_ts = in_archive.source->get()) != nullptr) {
      L_(info) << "search for in TS idx: " << in_archive.curr_ts->index();
      for (auto& out : output_sinks) {
        L_(info) << "in " << out.source_path;
        while (!found_beginning &&
               (out.curr_ts = out.source->get()) != nullptr) {
    ...
    L_(info) << "found first transmitted timeslice: "
             << in_archive.curr_ts->index();

    // from now on, all TS of in_archive.source have to be found in one of the
    // output archives
    do {
      L_(info) << "searching and checking TS idx: "
               << in_archive.curr_ts->index();
      bool found_ts = false;
      for (auto& out : output_sinks) {
        ...
        if (!found_ts) {
          ...
          out.source =
              make_unique<fles::TimesliceInputArchive>(out.source_path);
          out.curr_ts = nullptr;
...
int64_t Verificator::get_component_idx_of_microslice(
    shared_ptr<fles::Timeslice> ts, shared_ptr<fles::StorableMicroslice> ms) {
...
bool Verificator::verify_ts_metadata(vector<string> output_archive_paths,
                                     uint64_t* timeslice_cnt,
                                     uint64_t timeslice_size,
                                     uint64_t overlap_size,
                                     uint64_t timeslice_components) const {
  ...
  for (string& output_archive_path : output_archive_paths) {
    sem.wait();
    future<bool> handle = std::async(
        std::launch::async,
        [&archive_cnt_stat, &ts_cnt, output_archive_path, timeslice_size,
         timeslice_components, overlap_size, &sem] {
          ...
          std::shared_ptr<fles::StorableTimeslice> current_ts =
              nullptr;           // timeslice to search in
          ...
          try {
            ...
            L_(fatal) << "FAILED to verify metadata of: '"
                      << output_archive_path << "'" << endl
                      << err.what();
            sem.post();
            return false;
          }
          L_(info) << "Successfully verified metadata of: '"
                   << output_archive_path << "'. "
                   << archive_cnt_stat.fetch_sub(1, std::memory_order_relaxed) -
                          1
                   << " remaining ...";
          sem.post();
          return true;
...
string Verificator::format_time_seconds(uint64_t seconds) {
  ...
  sstream << hour << "h:" << setw(2) << setfill('0') << min << "m:" << setw(2)
          << setfill('0') << sec << "s";
  return sstream.str();
}

bool Verificator::find_ms_build_offset(string input_archive_path,
                                       vector<string> output_archive_paths,
  ...
  shared_ptr<fles::Timeslice> first_timeslice =
      ts_source.get(); // timeslice to search in
...
/**
 * @todo This method is bloated because of variables used to keep track of the
 * current validation progress and the printing of said progress or in case of a
 * failure, printing the failure. Would make sense to come up with a solution to
 * pull that out. Variables related to progress reports have "_stat" appended to
 * their name (stat = statistics)
 */
bool Verificator::verify_forward(vector<string> input_archive_paths,
                                 vector<string> output_archive_paths,
                                 uint64_t timeslice_cnt,
                                 uint64_t overlap,
                                 uint64_t start_idx) {
  atomic_uint64_t overall_ms_cnt_stat =
      0; // counts how many ms are validated - summed up accrross the whole
         // validation process
  atomic_uint64_t overlap_cnt_stat =
      0; // counts how many overlaps between ts components were checked
  atomic_uint64_t overall_components_cnt_stat =
      0; // counts how many timesliced were validated
  atomic_uint64_t archive_cnt_stat =
      0; // counts how many archives were validated
  const uint64_t log_interval_stat =
      100; // every n microslices we print a short progress report
  ...
  if (find_ms_build_offset(input_archive_paths[0], output_archive_paths,
                           offset)) {
  ...
  // fles::MergingSource<fles::TimesliceSource>
  // ts_source(std::move(ts_sources));

  const uint64_t epoch_start_stat =
  ...
  for (string& input_archive_path : input_archive_paths) {
    ...
    future<bool> handle = std::async(
        std::launch::async,
        [this, &input_archives_cnt_stat, &epoch_start_stat, &archive_cnt_stat,
         &overall_ms_cnt_stat, &overall_components_cnt_stat, &overlap_cnt_stat,
         &start_idx, input_archive_path, &output_archive_paths, &overlap,
         &timeslice_cnt, &sem] {
          uint64_t local_ts_cnt_stat =
              0; // counts how many timeslices were checked
          uint64_t local_ms_cnt_stat = 0; // counts the validated microslices of
                                          // this specific microslice archive
          uint64_t components_per_ts_stat =
              0; // will hold the number of components per ts to calculate
                 // progress in percent
          uint64_t overall_components_to_validate_stat = 0;
          stringstream err_sstr; // for error printing

          fles::MicrosliceInputArchive ms_archive(input_archive_path);
          shared_ptr<fles::StorableMicroslice> ms =
              nullptr; // microslice to search for

          int64_t compontent_idx = -1; // will hold the component index this
                                       // microslice archive corrolates to
          uint64_t ms_idx = 0;
          vector<unique_ptr<fles::TimesliceSource>> ts_sources = {};
          for (auto p : output_archive_paths) {
          ...
          fles::MergingSource<fles::TimesliceSource> ts_source(
              std::move(ts_sources));
          shared_ptr<fles::Timeslice> current_ts =
              ts_source.get(); // timeslice to search in
          components_per_ts_stat = current_ts->num_components();
          overall_components_to_validate_stat =
              components_per_ts_stat * timeslice_cnt;
          try {
            do {
              for (; ms_idx < current_ts->num_core_microslices(); ms_idx++) {
                ...
                if (!ms) {
                  err_sstr << "Failed to get next microslice:" << endl
                           << "Microslice #" << ms_idx << " in timeslice #"
                           << local_ts_cnt_stat
                           << " (TS idx: " << current_ts->index() << ")" << endl
                           << "Microslice archive: " << input_archive_path
                           << " microslice #" << local_ms_cnt_stat;
                  throw runtime_error(err_sstr.str());
                }

                // Here we figure out which component is associated to this
                // microslice archive file - we have to do this only once per
                // archive file
                if (compontent_idx < 0) {
                  compontent_idx =
                      get_component_idx_of_microslice(current_ts, ms);
                  if (compontent_idx < 0) {
                    err_sstr << "Could not associate timeslice component to "
                                "microslice:"
                             << endl
                             << "Microslice #" << ms_idx << " in timeslice #"
                             << local_ts_cnt_stat
                             << " (TS idx: " << current_ts->index() << ")"
                             << endl
                             << "Microslice archive: " << input_archive_path
                             << " microslice #" << local_ms_cnt_stat;
                    throw runtime_error(err_sstr.str());
                  }
                  L_(info) << "MS archive \"" << input_archive_path
                           << "\" corrolates to TS component: "
                           << compontent_idx;
                }

                fles::MicrosliceView ms_in_curr_component =
                    current_ts->get_microslice(compontent_idx, ms_idx);
                if (start_idx <= local_ms_cnt_stat) {
                  if (ms_in_curr_component != *ms) {
                    err_sstr
                        << "Found inequality in core area of timeslice:" << endl
                        << "Microslice #" << ms_idx << " in timeslice #"
                        << local_ts_cnt_stat
                        << " (TS idx: " << current_ts->index() << ")" << endl
                        << "Microslice archive: " << input_archive_path
                ...
                ++local_ms_cnt_stat;
                uint64_t last_overall_ms_cnt_stat =
                    overall_ms_cnt_stat.fetch_add(1);
                if (((last_overall_ms_cnt_stat + 1) % log_interval_stat) ==
                    0u) {
                  uint64_t epoch_now =
                  ...
                  double microslices_per_sec =
                      double(last_overall_ms_cnt_stat) / double(t_diff);
                  double progress =
                      (double(overall_components_cnt_stat) /
                       double(overall_components_to_validate_stat)) *
                      100.0;
                  double estimted_time = (100.0 / progress) * t_diff - t_diff;
                  L_(debug)
                      << "--- Status (" << STREAM_FLOAT(2, progress) << "% - "
                      << format_time_seconds(estimted_time)
                      << " remaining): ---" << endl
                      << last_overall_ms_cnt_stat + 1
                      << " microslices validated" << " ("
                      << STREAM_FLOAT(2, microslices_per_sec)
                      << " microslices per sec)" << endl
                      << archive_cnt_stat << "/" << input_archives_cnt_stat
                      << " input archives validated";
                }
              }

              shared_ptr<fles::Timeslice> next_ts = ts_source.get();

              if (!next_ts) { // if we have no follow up ts, we are in the very
                              // last ts - check the remaining ms in the current
                              // ts
                for (ms_idx = 0; ms_idx < overlap; ms_idx++) {
                  ms = ms_archive.get();
                  if (!ms) {
                    err_sstr << "Could not get next microslice from MS archive:"
                             << endl
                             << "Microslice #" << ms_idx << " in timeslice #"
                             << local_ts_cnt_stat
                             << " (TS idx: " << current_ts->index() << ")"
                  ...
                  fles::MicrosliceView ms_in_curr_component =
                      current_ts->get_microslice(
                          compontent_idx,
                          ms_idx + current_ts->num_core_microslices());
                  if (start_idx <= local_ms_cnt_stat) {
                    if (*ms != ms_in_curr_component) {
                      err_sstr
                          << "Found inequality in overlap of last timeslice:"
                          << endl
                          << "Microslice #" << ms_idx << " in timeslice #"
                          << local_ts_cnt_stat
                          << " (TS idx: " << current_ts->index() << ")" << endl
              ...
              } else { // else check the overlap-many *last* MS from the
                       // current_ts and overlap-many *first* MS from the
                       // next_ts - they have to be identical
                for (ms_idx = 0; ms_idx < overlap; ms_idx++) {
                  ms = ms_archive.get();
                  if (!ms) {
                    err_sstr << "Could not get next microslice from MS archive "
                                "while overlap checking:"
                             << endl
                             << "Microslice #" << ms_idx << " in timeslice #"
                             << local_ts_cnt_stat
                             << " (TS idx: " << current_ts->index() << ")"
                             << endl
                             << "Microslice archive: " << input_archive_path
                             << " microslice #" << local_ms_cnt_stat << endl;
                    throw runtime_error(err_sstr.str());
                  }
                  ...
                  fles::MicrosliceView ms_in_curr_component =
                      current_ts->get_microslice(
                          compontent_idx,
                          ms_idx + current_ts->num_core_microslices());
                  fles::MicrosliceView ms_in_next_component =
                      next_ts->get_microslice(compontent_idx, ms_idx);
                  if (start_idx <= local_ms_cnt_stat) {
                    if ((*ms != ms_in_curr_component ||
                         *ms != ms_in_next_component)) {
                      err_sstr
                          << "Overlap check failed:" << endl
                          << "Microslice archive: " << input_archive_path
                          << " microslice #" << local_ms_cnt_stat << endl
                          << "From timeslice #: " << local_ts_cnt_stat
                          << " (TS idx: " << current_ts->index() << ")" << endl
                          << "To timeslice #: " << (local_ts_cnt_stat + 1)
                          << " (TS idx: " << next_ts->index() << ")" << endl
                          << "MS in TS # " << local_ts_cnt_stat << ":" << endl
                          << MicrosliceDescriptorDump(
                                 ms_in_curr_component.desc())
                          << endl
                          << "MS in TS #:" << (local_ts_cnt_stat + 1) << ":"
                          << endl
                          << MicrosliceDescriptorDump(
                                 ms_in_next_component.desc())
                          << endl
                          << "MS in archive:" << endl
                          << MicrosliceDescriptorDump(ms->desc());
                      if (ms->content() != ms_in_curr_component.content()) {
                        err_sstr
                            << R"(Content of "Archive MS" and "Current TS MS" is different)"
                            << endl;
                      }

                      if (ms->content() != ms_in_next_component.content()) {
                        err_sstr
                            << R"(Content of "Archive MS" and "Next TS MS" is different)"
                            << endl;
                      }

                      if (ms_in_curr_component.content() !=
                          ms_in_next_component.content()) {
                        err_sstr
                            << R"(Content of "Current TS MS" and "Next TS MS" is different)"
                            << endl;
                      }
                      throw runtime_error(err_sstr.str());
                    }
  ...
  uint64_t epoch_now = chrono::duration_cast<std::chrono::seconds>(
                           chrono::system_clock::now().time_since_epoch())
                           .count();
