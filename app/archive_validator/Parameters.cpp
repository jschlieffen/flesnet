...
void Parameters::parse_options(int argc, char* argv[]) {
  ...
  general_add(
      "max-threads,j",
      po::value<uint64_t>(&max_threads)->default_value(1)->value_name("<n>"),
      "How many threads to use for validation. Set to 0 to use all available "
      "physical cores.");
  general_add("skip-metadata",
              po::value<bool>(&skip_metadata)->implicit_value(true),
              "Will skip timeslice archive metadata verification.");
  general_add(
      "ms-start-idx",
      po::value<uint64_t>(&ms_start_idx)->default_value(0)->value_name("<n>"),
      "Start verification process at microslice index n.");
  general_add("tsa-forwarding", po::bool_switch(&check_tsa_forwarding));

  po::options_description archives("Input/Output archives");
  auto archives_add = archives.add_options();
  archives_add("input-archives,I",
               po::value<vector<string>>(&input_archives)
                   ->multitoken()
                   ->value_name("<space-separated-archive-files>"),
               "Paths to input archives (.msa or .tsa based on use case).");
  archives_add("output-archives,O",
               po::value<vector<string>>(&output_archives)
                   ->multitoken()
  ...
  ts_parameters_add(
      "timeslice-cnt",
      po::value<uint64_t>(&timeslice_cnt)->default_value(0)->value_name("<n>"),
      "Summed up number of expected timeslices in the timeslice archives. "
      "Don't use this option for a more relaxed verification process.");
  ts_parameters_add("timeslice-size",
                    po::value<uint64_t>(&timeslice_size)->value_name("<n>"),
                    "Expected timeslice size.");
  ts_parameters_add(
      "overlap",
      po::value<uint64_t>(&overlap)->default_value(overlap)->value_name("<n>"),
      "Timeslice overlap size.");

  stringstream desc_sstr;
  desc_sstr << endl
            << "Usage:" << endl
            << "\tarchive_validator [options] -I input1.msa [ input2.msa ... ] "
               "-O output1.tsa [ output2.tsa ... ]"
            << endl
            << endl
            << "This program checks if the output data was generated by the "
               "given input data."
            << endl
            ...
            << endl
            << "Use the 'timeslice parameters' to define the expected "
               "structure of the timeslices in the provided .tsa files"
            << endl
            << endl
            << "Command line options";
  po::options_description desc(desc_sstr.str());
  desc.add(general).add(archives).add(ts_parameters);
  ...
  if (vm.count("help") != 0u) {
    cout << "archive_validator, git revision " << g_GIT_REVISION << endl;
    cout << desc << endl;
    exit(EXIT_SUCCESS);
  }

  if (vm.count("version") != 0u) {
    cout << "archive_validator " << g_PROJECT_VERSION_GIT << ", git revision "
         << g_GIT_REVISION << endl;
    exit(EXIT_SUCCESS);
  }
  ...
  if (!analyze_parameter_set) {
    throw ParametersException(
        "Not all necessary parameters for archive analyzing are set.");
  }
}
