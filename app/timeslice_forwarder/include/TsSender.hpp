...
private:
  ...
  std::atomic_uint64_t failed_remote_lock_cnt_ =
      0; // couts how many time remote buffer map lock failed
  std::atomic_uint64_t send_cnt_ =
      0; // counts how many tranmissions were performed
  std::atomic_uint64_t rem_buffer_full_cnt_ =
      0; // counts how many times the remote buffer was full when trying to
         // transmit data
  std::atomic_uint64_t rem_buffer_map_full_cnt_ =
      0; // counts how many times the remote buffer map was out of elements when
   ...
   */
  void on_node_connected(std::string address,
