...
public:
  ...
  /**
   * @brief Setter for the buffer map. The TsclientReader takes care of
   * representing new timeslices in the buffer map. so new timeslices can be
   * represented in it.
   */
  ...
  /**
   * @brief Register a callback which gets called when a new timeslice is
   * available.
   */
  void on_new_timeslice(std::function<void()> cb);

  /**
   * @brief Destroys the last read timeslice. Needs to be called so a new
   * timeslice can be received.
   */
  void clear_last_timeslice();

  /**
   * @brief Setter for the Connector used to transmit data, to lock the buffer
   * map.
   * @details If the receiver receives new TS this new data needs to be
   * represented in the buffer map. The connector provides the interface to lock
   * the buffer map.
   */
