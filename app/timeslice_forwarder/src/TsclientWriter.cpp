...
TsclientWriter::TsclientWriter(std::string output_uri, uint32_t timeslice_size)
  ...
  for (auto& [key, value] : uri.query_components) {
    if (key == "datasize") {
      ...
      throw runtime_error("Query parameter not implemented for scheme " +
                          uri.scheme + ": " + key);
    }
  }
  ...
  item_distributor_ = make_unique<ItemDistributor>(
      zmq_context_, producer_address_, worker_address_),
  ts_buffer_ = make_shared<FragmentedTimesliceBuffer>(
      zmq_context_, producer_address_, shm_identifier, datasize, descsize,
      num_components);
...
uint64_t TsclientWriter::get_buffer_size() { return buffer_size_; }
...
void TsclientWriter::write_timeslice(
    std::vector<BufferMap::ListElement*>& elements) {
  ...
  // L_(trace) << "write_timeslice - ts_input_output_diff: " <<
  // ++ts_input_output_cnt_diff  ;
  uint64_t combined_size = 0;
  for (auto desc_it = elements.begin(); desc_it != elements.end(); ++desc_it) {
    auto* const descriptor_el = *desc_it;
    if (1 == (descriptor_el->tag >>
              (sizeof(uint16_t) * 8))) { // referencing a descriptor
      desc_ptr.push_back(reinterpret_cast<fles::TimesliceComponentDescriptor*>(
          buffer_.get() + descriptor_el->address));
      const auto idx = static_cast<uint16_t>(descriptor_el->tag);
      for (const auto& element : elements) {
        if (static_cast<uint16_t>(element->tag) == idx &&
            2 == (element->tag >> (sizeof(uint16_t) * 8))) { // is refere
          combined_size += descriptor_el->len;
          combined_size += element->len;
          data_ptr.push_back(
              reinterpret_cast<uint8_t*>(buffer_.get() + element->address));
          break;
        }
      }
