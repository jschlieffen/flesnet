...
uint64_t TsclientReader::get_buffer_size() const { return buffer_size_; }
...
void TsclientReader::set_node_connector(
    std::shared_ptr<ConnectorInterface> node_connector) {
...
void TsclientReader::start_timeslice_reading() {
  ts_reading_thread_ = async([this] {
    ...
    auto addresses = shared_ptr<uint64_t>(new uint64_t[num_components_ * 2],
                                          default_delete<uint64_t[]>());
    auto sizes = shared_ptr<uint64_t>(new uint64_t[num_components_ * 2],
                                      default_delete<uint64_t[]>());
    auto tags = shared_ptr<uint32_t>(new uint32_t[num_components_ * 2],
    ...
    while (!stop_) {
      ...
      node_connector_->lock_buffer_map(
          buffer_map_, [&is_locked]() { is_locked = true; },
          []() { return true; });
      const auto num_components = tsf_timeslice->num_components();

      // tag layout:
      // [<is_descriptor or data> (uint16_t)] [data and descriptor have the same
      // int here (uint16_t)] in more detail:
      // ['1' is descriptor, '2' is data] [idx (set by loop variable)]
      // e.g.:
      // tag: [1][8] is a descriptor that belongs to the corrosponding data
      // element with tag [2][8]
      for (uint64_t i = 0; i < num_components; i++) {
        ...
        tags.get()[i] = static_cast<uint32_t>(1) << (sizeof(uint16_t) * 8) |
                        static_cast<uint16_t>(i); // descriptor has tag
        tags.get()[num_components + i] = static_cast<uint32_t>(2)
                                             << (sizeof(uint16_t) * 8) |
                                         static_cast<uint16_t>(i);

        addresses.get()[i] =
            reinterpret_cast<char*>(component_desc_ptr) - buffer_;
        addresses.get()[num_components + i] =
            reinterpret_cast<char*>(component_data_ptr) - buffer_;
      }
      ...
      if (buffer_map_ret == nullptr) {
        L_(fatal)
            << "(TimesliceReader) Buffer map full. Not handled yet - exiting";
        exit(-EXIT_FAILURE);
      }
