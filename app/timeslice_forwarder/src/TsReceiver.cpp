...
TsReceiver::TsReceiver(uint64_t node_id,
                       ...
                       std::string hostname)
    : Node(node_id, 2), cm_address_(central_manager_address),
  ...
  log_thread_ = std::async([this] {
    while (true) {
      ...
      L_(info) << "Input MB/s: "
               << (*bytes_received_.per_seconds() / 1000000.0);
    }
  });
  ...
  ts_sink_->on_timeslices_handled([this](uint64_t /*finished_ts_cnt*/) {
    ...
    node_connector_->lock_buffer_map(
        ...
        [this]() {
          ...
          L_(debug) << "TS on_timeslices_handled - done after: "
   ...
   * It seems to be constant 336 byte. Therefore this needs to be represented in
   * the buffer map too.
   */
  ...
  Node::on_new_work_item(
      std::bind(&TsReceiver::on_new_work_item, this, _1, _2, _3, _4, _5));
  Node::on_node_connected(
      std::bind(&TsReceiver::on_node_connected, this, _1, _2, _3));
  Node::on_node_disconnected(
      std::bind(&TsReceiver::on_node_disconnected, this, _1, _2, _3));
  Node::on_connection_refused(
      std::bind(&TsReceiver::on_connection_refused, this, _1));
  Node::start();
  Node::connect_to_node(cm_address_);
}

void TsReceiver::on_node_disconnected(std::string /*address*/,
                                      uint64_t group_id,
                                      uint64_t node_id) {
...
void TsReceiver::on_new_work_item(std::string /*address*/,
                                  std::shared_ptr<char> /*wi_ptr*/,
                                  WorkItem::Type /*wi_type*/,
                                  uint64_t group_id,
                                  uint64_t node_id) {
  L_(warning) << "Received work item from Node ID: " << node_id << " - "
              << " - Group ID: " << group_id
              << " - no handler implemented on receiver side";
}

void TsReceiver::on_node_connected(string address,
...
void TsReceiver::on_connection_refused(std::string address) {
  if (address == cm_address_) { // connection refused
    ...
    L_(fatal) << "Connection refused: Node with address '" << address
              << "' not available";
  }
};

void TsReceiver::on_new_data(const std::string& /*address*/,
                             uint64_t group_id,
                             uint64_t node_id) {
  ...
  node_connector_->lock_buffer_map(
      ...
      [this]() {
        ...
        if (el ==
            nullptr) { // not expected to happen in the current implementation
          node_connector_->unlock_buffer_map(data_buffer_map_);
          return;
        }
