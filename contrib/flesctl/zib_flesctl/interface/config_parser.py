...
class ParameterEditor(VerticalScroll):
    ...
    def compose(self):
        ...
        for group_name, group_parameters in self.parameters.groups.items():
            ...
            visible_parameters = [
                ...
                if param in self.parameters.display_params
            ]
    ...
    async def on_input_submitted(self, event: Input.Submitted):
        ...
        param = event.input.id.removeprefix(
            "param-"
        )
    ...
    async def on_checkbox_changed(self, event: Checkbox.Changed):
        ...
        param = event.checkbox.id.removeprefix(
            "param-"
        )

        if self.commit_parameter(
            param,
            event.value,
        ):
    ...
    def commit_parameter(self, param, value):
        ...
        try:
            current_value = (
                self.parameters.get_param_value(param)
            )
        ...
        section = self.parameters.get_param_section(
            param
        )
