...
class TimesliceBuilder : public Timeslice {

public:

  TimesliceBuilder(const TimesliceBuilder& ts);
  /// Delete assignment operator (not implemented).
  void operator=(const TimesliceBuilder&) = delete;
  /// Move constructor.
  TimesliceBuilder(TimesliceBuilder&& ts) noexcept;

  /// Construct by copying from given Timeslice object.
  TimesliceBuilder(const Timeslice& ts);

  /// Construct and initialize empty timeslice to fill using append_component.
  explicit TimesliceBuilder(uint32_t num_core_microslices,
