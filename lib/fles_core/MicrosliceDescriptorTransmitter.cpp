...
MicrosliceDescriptorTransmitter::MicrosliceDescriptorTransmitter(
    InputBufferWriteInterface& data_sink)
    : data_sink_(data_sink) {}

bool MicrosliceDescriptorTransmitter::try_put(
    const std::shared_ptr<const MicrosliceDescriptor>& item) {
  assert(item != nullptr);
  const DualIndex item_size = {1, item->size};
  ...
  data_sink_.desc_buffer().at(write_index_.desc) = *item;
...
void MicrosliceDescriptorTransmitter::put(std::shared_ptr<const MicrosliceDescriptor> item) {
  while (!try_put(item)) {
